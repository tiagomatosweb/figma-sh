<template>
  <div class="p-4">
    <div class="w-screen h-screen fixed top-0 left-0 z-40 bg-white flex items-center justify-center" v-if="loadScreen">
      <div
    ...
    <div class="grid grid-cols-4 gap-2 mb-4 items-end">
      <div class="flex flex-col gap-2">
        ...
        <Dropdown :title="currentConfig.theme">
          <span v-for="(theme, index) in config.themes" :key="index" @click="setConfig('theme', theme)"
      ...
      <div class="flex flex-col gap-2">
        ...
        <Dropdown :title="currentConfig.langTitle">
          <span v-for="(lang, index) in config.langs" :key="index" @click="setConfig('lang', lang.lang); setConfig('langTitle', lang.title)"
            class="cursor-pointer hover:bg-gray-900/75 px-2 py-1 rounded-md">
            {{ lang.title }}
          </span>
        </Dropdown>
